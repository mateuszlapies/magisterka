...
namespace Safety.Test
{
    public class BlockingTests
    {
        ...
        [Test]
        public void UnathorizedConfirmTest()
        {
            var userObjectOne = new User()
            {
                Name = "Test User 1"
            ...
            lockContext.Remove(postLink2.Id);
            lockContext.Lock(postLink1, postLink2, userTwo.GetPublicKey());
            lockContext.Confirm(postLink2.Id, userThree.GetOwner());
            var outcome = lockContext.Get(postLink1.Id);
            Assert.That(outcome.Lock.Confirmed, Is.Not.True);
            lockContext.Confirm(postLink2.Id, userTwo.GetOwner());
            outcome = lockContext.Get(postLink1.Id);
            Assert.That(outcome.Lock.Confirmed, Is.True);
        }
    }
}
